...
use git help command. $ git help
##### 3. How to check all the options of a command?
use git help command. $git command -help
...
-> fist login into your github account.
-> after that click on your profile and then go to settings.
-> in the setting click SSH and GPG keys.
-> click on the new ssh key.
-> then type the title and bottom of the page show  the add SSH key and click on that then we generate the SSH key
...
when we create new branch and make some changes and git checkout to new branch after that we use pull command to merge the main branch
command for pull is $ git pull origin <branch name>
...
to clone a repo from specific branch is $ git clone -b <branch name> <repo-url>
##### 27. How to roll back to a specific commit?
we use $ git checkout <commit id> 
roll back to previous commit $ git revert <commit id>
